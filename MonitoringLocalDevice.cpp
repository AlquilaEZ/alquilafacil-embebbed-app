...
#define ENDPOINT_SMOKE "https://alquilafacil-egde-node-fsaa.onrender.com/api/v1/edge-node/edge/readings/smoke"
#define ENDPOINT_RESTRICTED "https://alquilafacil-egde-node-fsaa.onrender.com/api/v1/edge-node/edge/readings/restricted-area"
#define ENDPOINT_ENTRY "https://alquilafacil-egde-node-fsaa.onrender.com/api/v1/edge-node/edge/readings/capacity"
#define ENDPOINT_SOUND "https://alquilafacil-egde-node-fsaa.onrender.com/api/v1/edge-node/edge/readings/noise"
...
MonitoringLocalDevice::MonitoringLocalDevice(int restrictedAreaPin, int smokeAnalogPin, int smokeDigitalPin, int innerMovementPin, int outerMovementPin)
...
void MonitoringLocalDevice::on(Event event) {
  ...
  if (event == EntryMovementSensor::INNER_MOVEMENT_DETECTED_EVENT){
    ...
    if(now - lastOuterSensorTriggerTime < MOVEMENT_TIMEOUT_MS){
      ...
      sendEventToServer("entry", "Se detecto la salida de una persona", ENDPOINT_ENTRY);
    } else {
  ...
  if (event == EntryMovementSensor::OUTER_MOVEMENT_DETECTED_EVENT){
    ...
    if(now - lastInnerSensorTriggerTime < MOVEMENT_TIMEOUT_MS){
      ...
      sendEventToServer("entry", "Se detecto la entrada de una persona",ENDPOINT_ENTRY);
    } else {
        lastOuterSensorTriggerTime = now;
    }
  }

  /*if (event == SoundSensor::SOUND_TRESHOLD_TRESPASSED_EVENT){
    //Para el sensor de sonido.
  }*/
}
